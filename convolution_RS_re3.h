...
//convolution layer3 design specially, failed, the bottolneck is loop2
//the result is worse, 11.07 ns
/********************************************/
...
//						#pragma HLS UNROLL
					...
					for(int to=0;to<pe_ROW;to++){//finish col conv
						#pragma HLS PIPELINE
...
//convolutionLayer for 3
template <typename T, int in_num, int in_DIM, int out_DIM, int k_DIM, int  out_num> \
void convolutionLayer3(T input[in_num][in_DIM][in_DIM], T weights[k_DIM* k_DIM* out_num], T bias[out_num], T output[out_num][out_DIM][out_DIM])
{
	const int k2_DIM=k_DIM*k_DIM;
	const int FACTOR=out_DIM/2;
	...
	#pragma HLS STREAM variable=kernel depth=25 dim=3

	T input_fifo[in_num][k2_DIM];
//	#pragma HLS STREAM variable=input_fifo depth=25 dim=2
...
//				output_temp[i][j][k] = 0;
	...
	#pragma HLS PIPELINE
		for(int i = 0; i<in_num; i++){
			#pragma HLS UNROLL
			for(int j=0; j<(k2_DIM); j++){ //input map fifo initial
				//#pragma HLS PIPELINE
				input_fifo[i][j] = input[i][int(floor(j/k_DIM))][j%k_DIM];
			}
			for(int j = 0; j<k2_DIM; j++){ //output map 
				//#pragma HLS PIPELINE
				output[to][0][0] += kernel[to][i][j] * input_fifo[i][j];
			}
		}
	}
...
//			#pragma HLS DATAFLOW
			...
			*/
			...
			output[to][0][0] += bias[to];
			ep = exp(output[to][0][0]);
			em = exp(-output[to][0][0]);
			output[to][0][0] = (ep-em)/(ep+em);
	}
}
...
//#pragma HLS PIPELINE
 	...
 	convolutionLayer3<float, 16, 5, 1, 5, 120>(layerOutput4, weights_layer5, bias_layer5, layerOutput5);
